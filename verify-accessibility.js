...
if (htmlContent.includes('Skip to main content') && htmlContent.includes('href="#main-content"')) {
...
const sectionsWithAriaLabelledby = (htmlContent.match(/aria-labelledby="/g) || []).length;
if (sectionsWithAriaLabelledby >= 3) {
  results.passed.push(`✓ Sections have aria-labelledby attributes (${sectionsWithAriaLabelledby} found)`);
} else {
  results.failed.push(`✗ Not enough sections with aria-labelledby (${sectionsWithAriaLabelledby} found, expected at least 3)`);
}

// Test 6: Focus utilities are present
const focusUtilities = (htmlContent.match(/focus:outline-none focus:ring-2/g) || []).length;
if (focusUtilities >= 5) {
  results.passed.push(`✓ Focus utilities are present (${focusUtilities} found)`);
} else {
  results.failed.push(`✗ Not enough focus utilities (${focusUtilities} found, expected at least 5)`);
}

// Test 7: External links have rel="noopener noreferrer"
const externalLinksWithRel = (htmlContent.match(/rel="noopener noreferrer"/g) || []).length;
const externalLinks = (htmlContent.match(/target="_blank"/g) || []).length;
if (externalLinksWithRel === externalLinks && externalLinks > 0) {
  results.passed.push(`✓ All external links have rel="noopener noreferrer" (${externalLinks} links)`);
} else {
  results.failed.push(`✗ Some external links are missing rel="noopener noreferrer" (${externalLinksWithRel}/${externalLinks})`);
}

// Test 8: Social media links have descriptive aria-labels
const socialLinksWithAriaLabel = (htmlContent.match(/aria-label="[^"]*"/g) || []).filter(label => 
  label.includes('GitHub') || label.includes('Twitter') || label.includes('LinkedIn') || label.includes('Email')
).length;
if (socialLinksWithAriaLabel >= 3) {
  results.passed.push(`✓ Social media links have descriptive aria-labels (${socialLinksWithAriaLabel} found)`);
} else {
  results.failed.push(`✗ Not enough social media links with aria-labels (${socialLinksWithAriaLabel} found, expected at least 3)`);
}
...
if (images.length > 0 && imagesWithAlt === images.length) {
  ...
  results.failed.push(`✗ Some images are missing alt text (${imagesWithAlt}/${images.length})`);
}

// Test 10: Semantic HTML elements are used
const semanticElements = ['<nav', '<main', '<section', '<article', '<header', '<footer'];
const foundSemanticElements = semanticElements.filter(el => htmlContent.includes(el));
if (foundSemanticElements.length >= 4) {
  results.passed.push(`✓ Semantic HTML elements are used (${foundSemanticElements.length} types found)`);
} else {
  results.failed.push(`✗ Not enough semantic HTML elements (${foundSemanticElements.length} found, expected at least 4)`);
}
