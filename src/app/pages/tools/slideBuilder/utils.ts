import { PX_TO_PT, type RichTextSegment } from './types';
...
export function parseGradient(bg: string): { angle: number; color1: string; color2: string } | null {
...
export function parseBoxShadow(shadow: string): { blur: number; offsetX: number; offsetY: number; color: string; opacity: number } | null {
  ...
  return {
    offsetX: parseFloat(parts[1]), offsetY: parseFloat(parts[2]), blur: parseFloat(parts[3]),
    color, opacity: opacityMatch ? parseFloat(opacityMatch[1]) : 0.3,
  };
}
...
export function extractRichText(el: HTMLElement, parentStyle: CSSStyleDeclaration): RichTextSegment[] | null {
