import { SLIDE_W, SLIDE_H, PX_TO_PT, type SlideElement } from './types';
    ...
    for (const el of elements) {
      ...
      if (el.type === 'shape') {
        ...
        const lineOpt: any = el.borderColor
          ...
          : el.borders ? undefined : { type: 'none' };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const opts: any = { ...p, ...rotOpt, ...shadowOpt, fill };
        if (lineOpt) opts.line = lineOpt;
        if (el.borderRadius && el.borderRadius > 0) opts.rectRadius = Math.min(el.borderRadius, Math.min(el.w, el.h) / 2);
        slide.addShape('rect', opts);

        if (el.borders) {
          const sides = [
            { b: el.borders[0], x1: p.x, y1: p.y, x2: p.x + p.w, y2: p.y },
            { b: el.borders[1], x1: p.x + p.w, y1: p.y, x2: p.x + p.w, y2: p.y + p.h },
            { b: el.borders[2], x1: p.x, y1: p.y + p.h, x2: p.x + p.w, y2: p.y + p.h },
      ...
      if (el.type === 'text') {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const baseOpts: any = { ...p, ...rotOpt, ...shadowOpt, align: el.align || 'left', valign: el.valign || 'top', wrap: true, lineSpacingMultiple: el.lineHeight || 1.4, margin, transparency };

        if (el.richText && el.richText.length > 0) {
          const segments = el.richText.map(seg => ({
            text: seg.text,
            options: {
              fontSize: seg.fontSize || 12, bold: seg.bold || false, italic: seg.italic || false,
              underline: seg.underline ? { style: 'sng' as const } : undefined,
              strike: seg.strike ? 'sngStrike' as const : undefined,
              color: seg.color || '333333', fontFace: seg.fontFamily || el.fontFamily || 'Yu Gothic',
              hyperlink: seg.hyperlink ? { url: seg.hyperlink } : undefined, breakLine: seg.breakAfter || false,
            },
          }));
          slide.addText(segments, baseOpts);
        } else if (el.text) {
          slide.addText(el.text, {
            ...baseOpts, fontSize: el.fontSize || 12, bold: el.fontBold || false, italic: el.fontItalic || false,
            underline: el.fontUnderline ? { style: 'sng' as const } : undefined,
            strike: el.fontStrike ? 'sngStrike' as const : undefined,
            color: el.fontColor || '333333', fontFace: el.fontFamily || 'Yu Gothic',
            charSpacing: el.charSpacing, hyperlink: el.hyperlink ? { url: el.hyperlink } : undefined,
          });
        }
      }
      ...
      if (el.type === 'list' && el.bullets?.length) {
        const rows = el.bullets.map(b => ({
          text: b.text,
          options: { fontSize: b.fontSize || 12, bold: b.bold, color: b.color || '333333', fontFace: el.fontFamily || 'Yu Gothic', bullet: el.listType === 'number' ? { type: 'number' } : true, indentLevel: b.indentLevel || 0 },
        }));
      ...
      if (el.type === 'table' && el.tableRows?.length) {
        const rows = el.tableRows.map((row, ri) =>
          row.map(cell => ({
            text: cell,
            options: { fontSize: el.fontSize || 10, fontFace: el.fontFamily || 'Yu Gothic', bold: ri === 0, fill: ri === 0 ? { color: 'E8E8E8' } : undefined, border: { type: 'solid', pt: 0.5, color: 'CCCCCC' } },
          }))
        );
      ...
      if (el.type === 'image' && el.imgSrc) {
        try {
          ...
          if (el.imgSrc.startsWith('data:')) { imgBase.data = el.imgSrc; } else { imgBase.path = el.imgSrc; }
          if (el.imgSizing === 'crop' || el.imgSizing === 'cover') { imgBase.sizing = { type: 'cover', w: p.w, h: p.h }; }
          else if (el.imgSizing === 'contain') { imgBase.sizing = { type: 'contain', w: p.w, h: p.h }; }
